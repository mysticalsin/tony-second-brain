...
Write to **`00_Inbox/from-dust/<agent>/`** (agent name matches `^[a-z0-9-]+$` — hyphens, never underscores). Every written `.md` MUST carry these **8 required** frontmatter fields (authoritative schema: [`99_Meta/sbap-schemas/agent_write.schema.json`](sbap-schemas/agent_write.schema.json)):
...
How every model here reasons, sources, refuses, and handles being wrong — the shared CONDUCT contract: **[99_Meta/agent-behavior-standard.md](agent-behavior-standard.md)**. It governs conduct (honesty, search discipline, sourcing/IP, legal-financial caution, evenhandedness, wellbeing, refusals); style still defers to your brand DNA and persona modes.
...
- **Never auto-send anything external.** Drafts only — the vault owner reviews. HR Documents/ and LinkedIn/ are off-limits; route PII / client-confidential content through the confidentiality guard.
